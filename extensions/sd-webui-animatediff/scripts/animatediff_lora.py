    ...
    def hack(self):
        ...
        def mm_load_network(name, network_on_disk):
            def convert_mm_name_to_compvis(key):
                ...
                sd_module_key = sd_module_key.replace("processor.", "").replace(
                    "to_out", "to_out.0"
                )
            ...
            if "motion_modules" in list(sd.keys())[0]:
                logger.info(
                    f"Loading motion lora {name} from {network_on_disk.filename}"
                )
                matched_networks = {}
                ...
                for key_network, weight in sd.items():
                    ...
                    assert (
                        sd_module is not None
                    ), f"Failed to find sd module for key {key}."

                    if key not in matched_networks:
                        matched_networks[key] = network.NetworkWeights(
                            network_key=key_network,
                            sd_key=key,
                            w={},
                            sd_module=sd_module,
                        )
