...
class AnimateDiffOutput:
    def output(
        ...
        step = (
            params.video_length
            if params.video_length > params.batch_size
        ...
        for i in range(res.index_of_first_image, len(res.images), step):
            ...
            video_list = [
                image.copy() for image in res.images[i : i + params.video_length]
            ]
    ...
    def _interp(
        ...
        try:
            ...
            logger.error(
                "Deforum not found. Please install: https://github.com/deforum-art/deforum-for-automatic1111-webui.git"
            )
            return video_list
        ...
        film_in_between_frames_count = calculate_frames_to_add(
            len(video_list), params.interp_x
        )
                    ...
                    filter_graph=(
                        {
                            ...
                            "scale": (
                                "scale",
                                f"{video_list[0].width}:{video_list[0].height}",
                            ),
                        },
                        [
                            ("video_in", "scale", 0, 0),
