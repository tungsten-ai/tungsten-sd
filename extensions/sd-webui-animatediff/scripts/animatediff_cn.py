...
from modules.processing import (  # StableDiffusionProcessingImg2Img, # TODO uncomment this
...
class AnimateDiffControl:
    ...
    def hack_batchhijack(self, params: AnimateDiffProcess):
        ...
        from scripts.batch_hijack import BatchHijack, InputMode, instance
            ...
            if len(units) > 0:
                ...
                for idx, unit in enumerate(units):
                    ...
                    if getattr(unit, "input_mode", InputMode.SIMPLE) == InputMode.BATCH:
                        if not (
                            isinstance(unit.batch_images, str)
                            and unit.batch_images != ""
                        ):
                            assert (
                                global_input_frames != ""
                            ), "No input images found for ControlNet module"
                    ...
                    if getattr(unit, "input_mode", InputMode.SIMPLE) == InputMode.BATCH:
                        if "inpaint" in unit.module:
                            ...
                            assert len(images) == len(
                                masks
                            ), "Inpainting image mask count mismatch"
                            unit.batch_images = [
                                {"image": images[i], "mask": masks[i]}
                ...
                if len(unit_batch_list) > 0:
                    ...
                    for unit in units:
                        if (
                            getattr(unit, "input_mode", InputMode.SIMPLE)
                            == InputMode.BATCH
                        ):
                            unit.batch_images = unit.batch_images[: params.video_length]

            return getattr(processing, "__controlnet_original_process_images_inner")(
                p, *args, **kwargs
            )

        self.original_processing_process_images_hijack = (
            BatchHijack.processing_process_images_hijack
        )
        BatchHijack.processing_process_images_hijack = (
    ...
    def hack_cn(self):
        ...
        def hacked_main_entry(self, p: StableDiffusionProcessing):
                ...
                if hasattr(p, "mask_blur_x"):
                    if getattr(p, "mask_blur_x", 0) > 0:
                        ...
                        np_mask = cv2.GaussianBlur(
                            np_mask, (kernel_size, 1), p.mask_blur_x
                        )
                    ...
                    if getattr(p, "mask_blur_y", 0) > 0:
                        ...
                        np_mask = cv2.GaussianBlur(
                            np_mask, (1, kernel_size), p.mask_blur_y
                        )
                        mask = Image.fromarray(np_mask)
                else:
            ...
            def set_numpy_seed(
                ...
                try:
                    tmp_seed = int(
                        p.all_seeds[0] if p.seed == -1 else max(int(p.seed), 0)
                    )
                    tmp_subseed = int(
                        p.all_seeds[0] if p.subseed == -1 else max(int(p.subseed), 0)
                    )
            ...
            for idx, unit in enumerate(self.enabled_units):
                ...
                if getattr(unit, "input_mode", InputMode.SIMPLE) == InputMode.BATCH:
                    ...
                    input_image, image_from_a1111 = cn_script.choose_input_image(
                        p, unit, idx
                    )
                ...
                for idx, input_image in enumerate(input_images):
                    a1111_mask_image: Optional[Image.Image] = getattr(
                        p, "image_mask", None
                    )
                    if (
                        ...
                        if a1111_mask.ndim == 2:
                            if a1111_mask.shape[0] == input_image.shape[0]:
                                if a1111_mask.shape[1] == input_image.shape[1]:
                                    ...
                                    a1111_i2i_resize_mode = getattr(
                                        p, "resize_mode", None
                                    )
                                    if a1111_i2i_resize_mode is not None:
                                        resize_mode = (
                                            external_code.resize_mode_from_value(
                                                a1111_i2i_resize_mode
                                            )
                                        )
                ...
                if (
                    ...
                    for idx, input_image in enumerate(input_images):
                        _, input_image = cn_script.detectmap_proc(
                ...
                if unit.pixel_perfect:
                    preprocessor_resolution = external_code.pixel_perfect_resolution(
                        input_images[0], target_H=h, target_W=w, resize_mode=resize_mode
                    )
                ...
                for idx, input_image in enumerate(input_images):
                    ...
                    if high_res_fix:
                        if is_image:
                            hr_control, hr_detected_map = cn_script.detectmap_proc(
                    ...
                    if is_image:
                        control, detected_map = cn_script.detectmap_proc(
                    ...
                    if control_model_type == ControlModelType.IPAdapter:
                        if model_net.is_plus:
                            controls_ipadapter["hidden_states"].append(
                                control["hidden_states"][-2]
                            )
                        else:
                            controls_ipadapter["image_embeds"].append(
                                control["image_embeds"]
                            )
                        if hr_control is not None:
                            if model_net.is_plus:
                                hr_controls_ipadapter["hidden_states"].append(
                                    hr_control["hidden_states"][-2]
                                )
                            else:
                                hr_controls_ipadapter["image_embeds"].append(
                                    hr_control["image_embeds"]
                                )
                        else:
                ...
                if control_model_type == ControlModelType.IPAdapter:
                    ipadapter_key = (
                        "hidden_states" if model_net.is_plus else "image_embeds"
                    )
                    controls = {
                        ipadapter_key: torch.cat(
                            controls_ipadapter[ipadapter_key], dim=0
                        )
                    }
                    if controls[ipadapter_key].shape[0] > 1:
                        controls[ipadapter_key] = torch.cat(
                            [controls[ipadapter_key], controls[ipadapter_key]], dim=0
                        )
                    if model_net.is_plus:
                    ...
                    if hr_controls_ipadapter is not None:
                        hr_controls = {
                            ipadapter_key: torch.cat(
                                hr_controls_ipadapter[ipadapter_key], dim=0
                            )
                        }
                        if hr_controls[ipadapter_key].shape[0] > 1:
                            hr_controls[ipadapter_key] = torch.cat(
                                [
                ...
                unit_is_batch = (
                    getattr(unit, "input_mode", InputMode.SIMPLE) == InputMode.BATCH
                )
                if "inpaint_only" in unit.module:
                    ...
                    for i in range(len(controls)):
                        final_inpaint_feed = (
                            hr_controls[i] if hr_controls is not None else controls[i]
                        )
                        final_inpaint_feed = final_inpaint_feed.detach().cpu().numpy()
                        final_inpaint_feed = np.ascontiguousarray(
                            final_inpaint_feed
                        ).copy()
                        final_inpaint_mask = final_inpaint_feed[0, 3, :, :].astype(
                            np.float32
                        )
                        final_inpaint_raw = final_inpaint_feed[0, :3].astype(np.float32)
                        sigma = shared.opts.data.get(
                            "control_net_inpaint_blur_sigma", 7
                        )
                        final_inpaint_mask = cv2.dilate(
                            final_inpaint_mask, np.ones((sigma, sigma), dtype=np.uint8)
                        )
                        final_inpaint_mask = cv2.blur(
                            final_inpaint_mask, (sigma, sigma)
                        )[None]
        ...
        self.cn_script.controlnet_main_entry = MethodType(
            hacked_main_entry, self.cn_script
        )
        self.cn_script.postprocess_batch = MethodType(
            hacked_postprocess_batch, self.cn_script
        )
