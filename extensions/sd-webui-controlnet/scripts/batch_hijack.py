...
from modules import processing, script_callbacks, shared  # img2img, # TODO img2img
...
class BatchHijack:
    ...
    #     if not cn_is_batch:
    #         return getattr(img2img, "__controlnet_original_process_batch")(
    #             p, *args, **kwargs
    #         )
    ...
    def processing_process_images_hijack(self, p, *args, **kwargs):
        ...
        try:
            self.dispatch_callbacks(
                self.process_batch_callbacks, p, batches, output_dir
            )
            ...
            for batch_i in range(self.batch_size):
                ...
                if shared.opts.data.get("controlnet_show_batch_images_in_ui", False):
                    output_images.extend(
                        processed.images[processed.index_of_first_image :]
                    )

                if output_dir:
                    self.save_images(
                        output_dir,
                        input_file_names[batch_i],
                        processed.images[processed.index_of_first_image :],
                    )
...
) -> Tuple[bool, List[List[str]], str, List[str]]:
