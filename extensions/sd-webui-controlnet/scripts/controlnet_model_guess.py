...
import torch
from scripts.adapter import Adapter, Adapter_light, PlugableAdapter, StyleAdapter
...
def build_model_by_guess(state_dict, unet, model_path):
    if "lora_controlnet" in state_dict:
        ...
        print("controlnet_sdxl_config (using lora)")
    ...
    model_has_shuffle_in_filename = (
        "shuffle" in Path(os.path.abspath(model_path)).stem.lower()
    )
    ...
    if "input_hint_block.0.weight" in state_dict:
        if "label_emb.0.0.bias" not in state_dict:
            ...
            print("controlnet_default_config")
            config["global_average_pooling"] = model_has_shuffle_in_filename
            config["hint_channels"] = int(
                state_dict["input_hint_block.0.weight"].shape[1]
            )
            config["context_dim"] = int(
                state_dict[
            ...
            has_full_layers = (
                "input_blocks.8.1.transformer_blocks.9.norm3.weight" in state_dict
            )
            has_mid_layers = (
                "input_blocks.8.1.transformer_blocks.0.norm3.weight" in state_dict
            )
            if has_full_layers:
                config = copy.deepcopy(controlnet_sdxl_config)
                print("controlnet_sdxl_config")
            elif has_mid_layers:
                config = copy.deepcopy(controlnet_sdxl_mid_config)
                print("controlnet_sdxl_mid_config")
            else:
                config = copy.deepcopy(controlnet_sdxl_small_config)
                print("controlnet_sdxl_small_config")
            config["global_average_pooling"] = False
            config["hint_channels"] = int(
                state_dict["input_hint_block.0.weight"].shape[1]
            )
    ...
    if "conv_in.weight" in state_dict:
        ...
        down_opts = tuple(
            filter(lambda item: item.endswith("down_opt.op.weight"), state_dict)
        )
    ...
    if "style_embedding" in state_dict:
        ...
        print("t2i_adapter_style_config")
    ...
    if "body.0.in_conv.weight" in state_dict:
        ...
        print("t2i_adapter_light_config")
