...
from ldm.modules.diffusionmodules.util import make_beta_schedule, timestep_embedding
...
def mark_prompt_context(x, positive):
    ...
    if isinstance(x, ScheduledPromptConditioning):
        if isinstance(x.cond, dict):
            ...
            return ScheduledPromptConditioning(
                end_at_step=x.end_at_step,
                cond=dict(crossattn=cond, vector=x.cond["vector"]),
            )
        else:
            cond = x.cond
            if prompt_context_is_marked(cond):
...
def unmark_prompt_context(x):
    if not prompt_context_is_marked(x):
        ...
        if not disable_controlnet_prompt_warning:
            print(
                "ControlNet Error: Failed to detect whether an instance is cond or uncond!"
            )
            print(
                "ControlNet Error: This is mainly because other extension(s) blocked A1111's \"process.sample()\" and deleted ControlNet's sample function."
            )
            print(
                "ControlNet Error: ControlNet will shift to a backup backend but the results will be worse than expectation."
            )
            print(
                "Solution (For extension developers): Take a look at ControlNet' hook.py "
                "UnetHook.hook.process_sample and manually call mark_prompt_context to mark cond/uncond prompts."
            )
        mark_batch = torch.ones(
            size=(x.shape[0], 1, 1, 1), dtype=x.dtype, device=x.device
        )
        uc_indices = []
        context = x
...
class AbstractLowScaleModel(nn.Module):
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        return (
            extract_into_tensor(self.sqrt_alphas_cumprod.to(x_start), t, x_start.shape)
            * x_start
            + extract_into_tensor(
                self.sqrt_one_minus_alphas_cumprod.to(x_start), t, x_start.shape
            )
            * noise
        )
...
def register_schedule(self):
    ...
    setattr(self, "sqrt_recip_alphas_cumprod", to_torch(np.sqrt(1.0 / alphas_cumprod)))
    setattr(
...
def predict_q_sample(ldm, x_start, t, noise=None):
    if noise is None:
        noise = torch.randn_like(x_start)
    return (
        extract_into_tensor(ldm.sqrt_alphas_cumprod.to(x_start), t, x_start.shape)
        * x_start
        + extract_into_tensor(
            ldm.sqrt_one_minus_alphas_cumprod.to(x_start), t, x_start.shape
        )
        * noise
    )
...
def predict_start_from_noise(ldm, x_t, t, noise):
    return (
        extract_into_tensor(ldm.sqrt_recip_alphas_cumprod.to(x_t), t, x_t.shape) * x_t
        - extract_into_tensor(ldm.sqrt_recipm1_alphas_cumprod.to(x_t), t, x_t.shape)
        * noise
    )
...
class UnetHook(nn.Module):
            ...
            if vae_output is None:
                with devices.autocast():
                    ...
                    if torch.all(torch.isnan(vae_output)).item():
                        print(
                            f"ControlNet find Nans in the VAE encoding. \n "
                ...
                print(
                    f"ControlNet used {str(devices.dtype_vae)} VAE to encode {vae_output.shape}."
                )
            latent = vae_output
    ...
    def guidance_schedule_handler(self, x):
        for param in self.control_params:
            ...
            param.guidance_stopped = (
                current_sampling_percent < param.start_guidance_percent
                or current_sampling_percent > param.stop_guidance_percent
            )
    ...
    def hook(self, model, sd_ldm, control_params, process):
        ...
        def forward(self, x, timesteps=None, context=None, y=None, **kwargs):
            ...
            cond_mark, outer.current_uc_indices, context = unmark_prompt_context(
                context
            )
            ...
            no_high_res_control = is_in_high_res_fix and shared.opts.data.get(
            ...
            for param in outer.control_params:
                if param.used_hint_cond_latent is not None:
                    continue
                if (
                    param.control_model_type
                    not in [ControlModelType.AttentionInjection]
            ...
            for param in outer.control_params:
                ...
                if param.control_model_type not in [
                ...
                ), f"Controlnet is enabled but no input image is given"
                ...
                control = param.control_model(
                    x=x_in, hint=hint, timesteps=timesteps, context=context, y=y
                )
            ...
            for param in outer.control_params:
                ...
                if param.control_model_type not in [
                    ControlModelType.AttentionInjection
                ]:
                    continue
        ...
        if need_attention_hijack:
            ...
            for i, module in enumerate(gn_modules):
                ...
                module.forward = hacked_group_norm_forward.__get__(
    ...
    def restore(self):
        scripts.script_callbacks.remove_callbacks_for_function(
            self.guidance_schedule_handler
        )
        self.control_params = None
