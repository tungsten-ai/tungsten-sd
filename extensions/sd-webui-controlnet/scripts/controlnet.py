...
from scripts.hook import (ControlModelType, ControlParams, HackedImageRNG,
                          UnetHook)
...
def find_closest_lora_model_name(search: str):
    ...
    applicable = [
        name for name in global_state.cn_models_names.keys() if search in name.lower()
    ]
    if not applicable:
        return None
...
def image_has_mask(input_image: np.ndarray) -> bool:
    ...
    return (
        input_image.ndim == 3
        and input_image.shape[2] == 4
        and np.max(input_image[:, :, 3]) > 127
    )
...
class Script(
    ...
    def __init__(self) -> None:
        ...
        self.preprocessor = global_state.cache_preprocessors(
        ...
        batch_hijack.instance.process_batch_each_callbacks.append(
            self.batch_tab_process_each
        )
        batch_hijack.instance.postprocess_batch_each_callbacks.insert(
            0, self.batch_tab_postprocess_each
        )
        batch_hijack.instance.postprocess_batch_callbacks.insert(
            0, self.batch_tab_postprocess
        )
    ...
    @staticmethod
    def get_default_ui_unit(is_ui=False):
    ...
    def load_control_model(p, unet, model):
        ...
        if len(Script.model_cache) > 0 and len(
            Script.model_cache
        ) >= shared.opts.data.get("control_net_model_cache_size", 2):
    ...
    def detectmap_proc(detected_map, module, resize_mode, h, w):
        ...
        def high_quality_resize(x, size):
            ...
            if x.shape[0] != size[1] or x.shape[1] != size[0]:
                ...
                if is_binary:
                    y = (
                        np.mean(y.astype(np.float32), axis=2)
                        .clip(0, 255)
                        .astype(np.uint8)
                    )
                    if is_one_pixel_edge:
                        y = nake_nms(y)
                        _, y = cv2.threshold(
                            y, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU
                        )
    ...
    def choose_input_image(
        ...
        if (
            batch_hijack.instance.is_batch
            and getattr(p, "image_control", None) is not None
        ):
            ...
            if "inpaint" in unit.module:
                ...
                if have_mask and not shared.opts.data.get(
    ...
    def bound_check_params(unit: external_code.ControlNetUnit) -> None:
        ...
        for param, default_value in defaults.items():
            ...
            if value < 0:
                ...
                print(
                    f"[{unit.module}.{param}] Invalid value({value}), using default value {default_value}."
                )
    ...
    def controlnet_main_entry(self, p):
        ...
        for idx, unit in enumerate(self.enabled_units):
            ...
            if (
                "inpaint" in unit.module
                and not image_has_mask(input_image)
                    ...
                    if a1111_mask.shape[0] == input_image.shape[0]:
                        if a1111_mask.shape[1] == input_image.shape[1]:
                            input_image = np.concatenate(
                                [input_image[:, :, 0:3], a1111_mask[:, :, None]], axis=2
                            )
                            a1111_i2i_resize_mode = getattr(p, "resize_mode", None)
                            if a1111_i2i_resize_mode is not None:
                                resize_mode = external_code.resize_mode_from_value(
            ...
            if (
                unit.module == "inpaint_only+lama"
                and resize_mode == external_code.ResizeMode.OUTER_FIT
            ):
                # inpaint_only+lama is special and required outpaint fix
                _, input_image = Script.detectmap_proc(
                    input_image, unit.module, resize_mode, hr_y, hr_x
                )
            ...
            elif hasattr(model_net, "control_model") and (
                isinstance(model_net.control_model, Adapter)
                or isinstance(model_net.control_model, Adapter_light)
            ):
                control_model_type = ControlModelType.T2I_Adapter
            elif hasattr(model_net, "control_model") and isinstance(
                model_net.control_model, StyleAdapter
            ):
            ...
            if unit.pixel_perfect:
                preprocessor_resolution = external_code.pixel_perfect_resolution(
                    input_image, target_H=h, target_W=w, resize_mode=resize_mode
                )
            ...
            if high_res_fix:
                if is_image:
                    hr_control, hr_detected_map = Script.detectmap_proc(
                        detected_map, unit.module, resize_mode, hr_y, hr_x
                    )
            ...
            if is_image:
                control, detected_map = Script.detectmap_proc(
                    detected_map, unit.module, resize_mode, h, w
                )
            ...
            if "inpaint_only" in unit.module:
                ...
                final_inpaint_mask = cv2.dilate(
                    final_inpaint_mask, np.ones((sigma, sigma), dtype=np.uint8)
                )
                final_inpaint_mask = cv2.blur(final_inpaint_mask, (sigma, sigma))[None]
                _, Hmask, Wmask = final_inpaint_mask.shape
                final_inpaint_raw = torch.from_numpy(
                    np.ascontiguousarray(final_inpaint_raw).copy()
                )
                final_inpaint_mask = torch.from_numpy(
                    np.ascontiguousarray(final_inpaint_mask).copy()
                )
            ...
            if "+lama" in unit.module:
                forward_param.used_hint_cond_latent = (
                    hook.UnetHook.call_vae_using_process(p, control)
                )
        ...
        self.latest_network.hook(
            model=unet, sd_ldm=sd_ldm, control_params=forward_params, process=p
        )
    ...
    def controlnet_hack(self, p):
        ...
        if len(self.enabled_units) > 0:
            print(f"ControlNet Hooked - Time = {time.time() - t}")
        return
    ...
    def before_process_batch(self, p, *args, **kwargs):
        if self.noise_modifier is not None:
            p.rng = HackedImageRNG(
                rng=p.rng, noise_modifier=self.noise_modifier, sd_model=p.sd_model
            )
        self.noise_modifier = None
    ...
    def postprocess(self, p, processed, *args):
        ...
        processor_params_flag = (
            ", ".join(getattr(processed, "extra_generation_params", []))
        ).lower()
        self.post_processors = []
        ...
        if (
            shared.opts.data.get("control_net_detectmap_autosaving", False)
            and self.latest_network is not None
        ):
            for detect_map, module in self.detected_map:
                detectmap_dir = os.path.join(
                    shared.opts.data.get("control_net_detectedmap_dir", ""), module
                )
                ...
                if "sd upscale" not in processor_params_flag:
                    if self.detected_map is not None:
                        for detect_map, module in self.detected_map:
                            ...
                            detect_map = external_code.visualize_inpaint_mask(
                                detect_map
                            )
    ...
    def batch_tab_process_each(self, p, *args, **kwargs):
        for unit_i, unit in enumerate(self.enabled_units):
            if (
                getattr(unit, "loopback", False)
                and batch_hijack.instance.batch_index > 0
            ):
                continue
    ...
    def batch_tab_postprocess_each(self, p, processed, *args, **kwargs):
        for unit_i, unit in enumerate(self.enabled_units):
            if getattr(unit, "loopback", False):
                output_images = getattr(processed, "images", [])[
...
def on_ui_settings():
    ...
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            global_state.default_detectedmap_dir,
    ...
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            3,
            "Multi-ControlNet: ControlNet unit number (requires restart)",
            section=section,
        ),
    )
    shared.opts.add_option(
        "control_net_model_cache_size",
        shared.OptionInfo(1, "Model cache size (requires restart)", section=section),
    )
    shared.opts.add_option(
        "control_net_inpaint_blur_sigma",
        shared.OptionInfo(
            7, "ControlNet inpainting Gaussian blur sigma", section=section
        ),
    )
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            False,
            "Do not apply ControlNet during highres fix",
            section=section,
        ),
    )
    shared.opts.add_option(
    ...
    shared.opts.add_option(
        "controlnet_disable_control_type",
        shared.OptionInfo(
            False,
            "Disable control type selection",
            section=section,
        ),
    )
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            False,
            "Disable openpose edit",
            section=section,
        ),
    )
    shared.opts.add_option(
        "controlnet_ignore_noninpaint_mask",
        shared.OptionInfo(
            False,
            "Ignore mask on ControlNet input image if control type is not inpaint",
            section=section,
        ),
    )
