...
from scripts import global_state, hook, external_code, processor, batch_hijack, utils
...
class Script(scripts.Script, metaclass=type):
    ...
    def bound_check_params(unit: external_code.ControlNetUnit) -> None:
        ...
        for param, default_value in defaults.items():
            ...
            if value < 0:
                ...
                print(f'[{unit.module}.{param}] Invalid value({value}), using default value {default_value}.')
    ...
    def process(self, p, *args):
        ...
        if is_sdxl:
            print('ControlNet does not support SDXL -- disabling')
            return
...
def on_ui_settings():
    ...
    shared.opts.add_option("control_net_max_models_num", shared.OptionInfo(
        3, "Multi ControlNet: Max models amount (requires restart)", section=section))
    shared.opts.add_option("control_net_model_cache_size", shared.OptionInfo(
        1, "Model cache size (requires restart)", section=section))
    ...
    shared.opts.add_option("control_net_no_high_res_fix", shared.OptionInfo(
        False, "Do not apply ControlNet during highres fix", section=section))
    shared.opts.add_option("control_net_no_detectmap", shared.OptionInfo(
        False, "Do not append detectmap to output", section=section))
    shared.opts.add_option("control_net_detectmap_autosaving", shared.OptionInfo(
        False, "Allow detectmap auto saving", section=section))
    shared.opts.add_option("control_net_allow_script_control", shared.OptionInfo(
        False, "Allow other script to control this extension", section=section))
    shared.opts.add_option("control_net_sync_field_args", shared.OptionInfo(
        False, "Passing ControlNet parameters with \"Send to img2img\"", section=section))
    shared.opts.add_option("controlnet_show_batch_images_in_ui", shared.OptionInfo(
        False, "Show batch images in gradio gallery output", section=section))
    shared.opts.add_option("controlnet_increment_seed_during_batch", shared.OptionInfo(
        False, "Increment seed after each controlnet batch iteration", section=section))
    shared.opts.add_option("controlnet_disable_control_type", shared.OptionInfo(
        False, "Disable control type selection", section=section))
    shared.opts.add_option("controlnet_disable_openpose_edit", shared.OptionInfo(
        False, "Disable openpose edit", section=section))
