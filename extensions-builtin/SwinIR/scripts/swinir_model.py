...
class UpscalerSwinIR(Upscaler):
    ...
    def do_upscale(self, img, model_file):
        use_compile = (
            hasattr(opts, "SWIN_torch_compile")
            and opts.SWIN_torch_compile
...
def on_ui_settings():
    shared.opts.add_option(
        "SWIN_tile", shared.OptionInfo(192, "Tile size for all SwinIR.")
    )
    shared.opts.add_option(
        "SWIN_tile_overlap",
        shared.OptionInfo(
            8, "Tile overlap, in pixels for SwinIR. Low values = visible seam."
        ),
    )
    if (
        int(torch.__version__.split(".")[0]) >= 2 and platform.system() != "Windows"
    ):  # torch.compile() require pytorch 2.0 or above, and not on Windows
        shared.opts.add_option(
            "SWIN_torch_compile",
            shared.OptionInfo(False, "Use torch.compile to accelerate SwinIR.").info(
