...
class UpscalerLDSR(Upscaler):
    ...
    def load_model(self, path: str):
        ...
        local_ckpt_path = next(
            iter(
                [
                    ...
                    if local_model.endswith("model.ckpt")
                ]
            ),
            None,
        )
        local_safetensors_path = next(
            iter(
                [
                    local_model
                    for local_model in local_model_paths
                    if local_model.endswith("model.safetensors")
                ]
            ),
            None,
        )
        local_yaml_path = next(
            iter(
                [
                    local_model
                    for local_model in local_model_paths
                    if local_model.endswith("project.yaml")
                ]
            ),
            None,
        )
        ...
        if local_safetensors_path is not None and os.path.exists(
