...
class CheckpointInfo:
    def __init__(self, filename):
        ...
        self.ids = [
            ...
            [self.shorthash, self.sha256, f"{self.name} [{self.shorthash}]"]
            if self.shorthash
            else []
        )
...
def load_model_weights(model, checkpoint_info: CheckpointInfo, state_dict, timer):
    ...
    devices.dtype_unet = (
        torch.float16
        if model.is_sdxl and not shared.cmd_opts.no_half
        else model.model.diffusion_model.dtype
    )
...
def repair_config(sd_config):
    ...
    if (
        getattr(
            sd_config.model.params.first_stage_config.params.ddconfig, "attn_type", None
        )
        == "vanilla-xformers"
