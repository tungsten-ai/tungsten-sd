...
from modules import sd_samplers_cfg_denoiser, sd_samplers_common, sd_samplers_extra
...
samplers_k_diffusion = [
    ...
    (
        ...
        "sample_dpm_2",
        ["k_dpm_2"],
        {"discard_next_to_last_sigma": True, "second_order": True},
    ),
    (
        "DPM2 a",
...
class KDiffusionSampler(sd_samplers_common.Sampler):
    ...
    def get_sigmas(self, p, steps):
        ...
        if p.sampler_noise_scheduler_override:
            ...
            default_rho = 1.0 if opts.k_sched_type == "polyexponential" else 7.0

            if (
                opts.k_sched_type != "exponential"
                and opts.rho != 0
                and opts.rho != default_rho
            ):
            ...
            self.config is not None
            and self.config.options.get("scheduler", None) == "karras"
        ):
            sigma_min, sigma_max = (
                (0.1, 10)
                if opts.use_old_karras_scheduler_sigmas
            ...
            sigmas = k_diffusion.sampling.get_sigmas_karras(
                n=steps, sigma_min=sigma_min, sigma_max=sigma_max, device=shared.device
            )
        elif (
            self.config is not None
            and self.config.options.get("scheduler", None) == "exponential"
        ):
            m_sigma_min, m_sigma_max = (
                self.model_wrap.sigmas[0].item(),
                self.model_wrap.sigmas[-1].item(),
